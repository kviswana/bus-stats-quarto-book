...
As we saw from @sec-mean-model, the *best* model would be to predict the average. Do you recall how to compute the average?
...
You might recall from @sec-missing-values that *NA* means missing value. Why is the mean a missing value? The *txhousing* data frame has some missing values in the column *listings*. As a result, it is unable to compute the average. Suppose I asked you what is 2 + 3, you can say "5." since you have both the numbers that you want to add up. 
...
In the earlier chapters, we have built 
This can get cumbersome as we will need to build the model differently depending on the type of explanatory variable. Fortunately, the *LSTbook* package provides a single function -- the *model_train* function through which we can build all the models for this course.
